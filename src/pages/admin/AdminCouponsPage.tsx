...
export function AdminCouponsPage() {
  ...
  const [form, setForm] = useState({ code: '', type: 'percentage', value: 10, min_order: 0, max_discount: 0, usage_limit: 0, expires_at: '', enabled: true });
  ...
  const handleSave = async () => {
    ...
    await supabase.from('coupons').insert({
      ...
      usage_limit: form.usage_limit > 0 ? form.usage_limit : null,
      expires_at: form.expires_at || null,
      enabled: form.enabled,
    });
    setSaving(false);
    setShowForm(false);
    setForm({ code: '', type: 'percentage', value: 10, min_order: 0, max_discount: 0, usage_limit: 0, expires_at: '', enabled: true });
    fetchData();
  };
            ...
            <tbody>
              {coupons.map((c) => (
                <tr key={c.id} className="border-b border-ink-50">
                  ...
                  <td className="p-4 text-ink-600">{c.usage_limit != null ? `${c.used_count ?? 0}/${c.usage_limit}` : 'Unlimited'}</td>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative w-full max-w-md rounded-xl bg-white p-6 animate-scale-in">
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div><label className="mb-1 block text-sm font-medium text-ink-700">Max Discount</label><input type="number" className="w-full rounded-lg border border-ink-200 px-4 py-2.5 text-sm" value={form.max_discount} onChange={(e) => setForm({ ...form, max_discount: Number(e.target.value) })} /></div>
                <div><label className="mb-1 block text-sm font-medium text-ink-700">Usage Limit</label><input type="number" className="w-full rounded-lg border border-ink-200 px-4 py-2.5 text-sm" value={form.usage_limit} onChange={(e) => setForm({ ...form, usage_limit: Number(e.target.value) })} /></div>
              </div>
