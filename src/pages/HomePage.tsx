  ...
  return (
    <div>
      ...
      <section className="mx-auto max-w-7xl px-3 py-10 sm:px-4 sm:py-16">
        ...
        <div className="h-[360px] overflow-hidden rounded-2xl bg-white sm:h-[460px]">
          <CircularGallery
        ...
        <div className="mt-4 flex flex-wrap justify-center gap-x-5 gap-y-2">
          {categories.slice(0, 6).map((cat) => (
            <Link key={cat.id} to={`/category/${cat.slug}`} className="text-xs font-semibold uppercase tracking-[0.12em] text-ink-500 transition-colors hover:text-accent-600">
              {cat.name}
            </Link>
          ))}
        </div>
